function synapse = r_exc(indexR,w,t)
...
if j0(indexR) > length(w)
    ...
    if t >= te0 && t < 0.0
        ...
        if SYNAPSE_exc(indexR) == 1 && (t-t2(indexR)) > pd1
        ...
        if SYNAPSE_exc(indexR) > 0
            synapse = r_inf_exc+(r0_exc(indexR)-r_inf_exc)*exp(-(1/tau_R_exc)*(t-t2(indexR)));
        else
            synapse = r0_exc(indexR)*exp(-beta_R_exc*(t-t2(indexR)));
        end
    else
        synapse = 0.0;
    end
